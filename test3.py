# test.py
# 👀 Gaze All-in-One — Webcam / ESP32 / Calibration Overlay
# ✅ รวม:
#   - Quick-wins 1–5 (Kalman iris, gradient-consistency, monolid-aware threshold,
#     dynamic ROI, HUD det-rate) + target-anchored calibration
#   - NEW: Bias/Gain Compensation (per-axis) คำนวณจาก residual หลังคาลิเบรต
#          พร้อม Toggle + Strength (alpha) ใน Sidebar และบันทึก/โหลดในโปรไฟล์
#
# Requires: streamlit, streamlit-webrtc, opencv-python(-headless), mediapipe, av, numpy
# Optional: pyautogui

import math, time, threading, queue, json, pathlib
...
class MouseController:
    ...
    def set_enable(self, v: bool):
        self.enabled = v
        if not v: self._last_in_time = None; self._last_target = None
...
class OneEuroFilter:
    ...
    def filter(self, x, t=None):
        if self.t_prev is None:
            self.t_prev=time.time() if t is None else t; self.x_prev=x; return x
        tnow=time.time() if t is None else t; dt=max(1e-6, tnow-self.t_prev)
        self.freq=1.0/dt; self.t_prev=tnow
        dx=(x-self.x_prev)*self.freq
        a_d=self.alpha(self.dcutoff); dx_hat=a_d*dx+(1-a_d)*self.dx_prev
        cutoff=self.mincutoff+self.beta*abs(dx_hat)
        a=self.alpha(cutoff); x_hat=a*x+(1-a)*self.x_prev
...
class SessionState:
    def __init__(self):
        ...
        self.calib_X=[]; self.calib_yx=[]; self.calib_yy=[]; self.calib_w=[]
        self.model_ready=False; self.model_pipeline=None
        self.last_feat: Optional[np.ndarray]=None
        self.last_quality: float = 0.5
        self.t_prev=None; self.fps_hist=[]
        self.drift_enabled=False; self.drift_dx=0.0; self.drift_dy=0.0; self.drift_alpha=0.15
        self.calib_report: Optional[CalibrationReport]=None
        # NEW: compensation params & control
        self.comp_ax=1.0; self.comp_bx=0.0
        self.comp_ay=1.0; self.comp_by=0.0
        self.comp_valid=False
        self.comp_alpha=1.0   # strength 0..1
...
def _feat_vec(feat: dict)->np.ndarray:
    ex=float(feat.get('eye_cx_norm',0.5)); ey=float(feat.get('eye_cy_norm',0.5))
    fx=float(feat.get('face_cx_norm',0.5)); fy=float(feat.get('face_cy_norm',0.5))
...
def _fit_weighted_ridge(Phi,y,sample_w,pen_vec,lam):
    ...
    Phi_w=Phi*sw; y_w=y.reshape(-1,1)*sw
...
class FeatureExtractor:
    LEFT_EYE_IDS=[33,133,159,145]; RIGHT_EYE_IDS=[362,263,386,374]
    ...
    def __init__(self, use_mediapipe: bool=True):
        ...
        if self.use_mediapipe:
            ...
            self.mesh=self.mp_face.FaceMesh(static_image_mode=False, refine_landmarks=True,
                                            max_num_faces=1, min_detection_confidence=0.5,
                                            min_tracking_confidence=0.5)
        else:
            self.mesh=None
        self.earL_base=None; self.earR_base=None; self._ema_alpha=0.05
    def close(self):
    ...
    def _extract_mediapipe(self, frame_bgr)->Optional[dict]:
        ...
        if l_iris is None and r_iris is None: return None
        ...
        if earL is not None and earL>0.18: self.earL_base=_upd_base(earL,self.earL_base)
        if earR is not None and earR>0.18: self.earR_base=_upd_base(earR,self.earR_base)
        def _eye_conf(ear,base,box,iris_ok):
            if (ear is None) or (box is None) or (not iris_ok): return 0.0
            ratio=(ear/0.26) if base is None else (ear/max(1e-6,0.8*base))
            ratio=max(0.0,min(1.4,ratio))
            _,_,_,_,bw,bh,_=box; area_norm=(bw*bh)/(w*h)
        ...
        wL,wR=cL,cR
        if (wL+wR)<1e-3: eye_cx_norm=0.5; eye_cy_norm=0.5
        else:
        ...
        return {"eye_cx_norm":eye_cx_norm,"eye_cy_norm":eye_cy_norm,
...
class Kalman2D:
    ...
    def predict(self, dt=None):
        t=time.time()
        if dt is None: dt=max(1e-3, t-self.last_t)
        self.last_t=t
...
def _iris_center_norm_from_frame(frame_bgr):
    ...
    x0=max(0, cx_prev - roi//2); y0=max(0, cy_prev - roi//2)
    x1=min(w, x0+roi); y1=min(h, y0+roi)
    x0=max(0, min(x0, w-(x1-x0))); y0=max(0, min(y0, h-(y1-y0)))
    x1=min(w, x0+roi); y1=min(h, y0+roi)
    roi_img=frame[y0:y1, x0:x1]
    if roi_img.size==0: roi_img=frame; x0=y0=0; x1=w; y1=h
    ...
    hi=float(np.percentile(gray, 96)); med=float(np.median(gray))
    glint_mask=gray>hi
    if np.any(glint_mask):
        gray=gray.copy(); gray[glint_mask]=int(med)
    gX=float(np.mean(np.abs(gx))); gY=float(np.mean(np.abs(gy)))
    eyelid_heavy = (gY > 1.3 * max(1e-3, gX))
    p_base=20; off_base=8
    if eyelid_heavy: p = max(15, p_base-2); off = off_base + 2
    else: p = p_base; off = off_base
    med_val=float(np.median(gray)); p20=float(np.percentile(gray,20))
    if (med_val - p20) < 10: off=max(5, off-2)
    ...
    best=None; best_score=-1.0; best_axes=None
    max_area=(roi*roi*0.55)
    for c in cnts:
        area=cv2.contourArea(c)
        if area<300 or area>max_area: continue
        if len(c)>=5:
            ...
            gc_vals=[]; cx_i, cy_i = float(cx_r), float(cy_r)
            ...
            if score>best_score:
                best_score=score; best=(int(cx_r), int(cy_r)); best_axes=(MA, ma)
    method="ellipse"
    used_darkest=False
    if best is None:
        try:
            circles=cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, dp=1.5, minDist=30,
                                     param1=60, param2=18, minRadius=8, maxRadius=int(roi*0.45))
            if circles is not None and len(circles[0])>0:
                c0=circles[0][0]; best=(int(c0[0]), int(c0[1])); best_axes=(2*c0[2], 2*c0[2]); method="hough"
        except Exception: pass
    if best is None:
        gray_full=cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        step=10; win=20; best_sum=1e18; bx=by=None
        ...
        cx, cy = bx, by; method="darkest"; used_darkest=True
    else:
        cx, cy = (x0 + best[0], y0 + best[1])
    now=time.time(); dt=max(1e-3, now - st["last_ts"]); st["last_ts"]=now
    if st["kf"] is None: st["kf"]=Kalman2D(cx, cy, 0.0, 0.0)
    st["kf"].predict(dt=dt)
    meas_scale = 2.5 if used_darkest else 1.0
    ...
    if best_score>0 and not used_darkest:
        ...
        if method=="hough": conf*=0.9
    else:
        conf=0.25
    if conf < 0.45 or used_darkest: st["roi"] = min(320, int(st["roi"]*1.12 + 6))
    elif conf > 0.65 and method in ("ellipse","hough"): st["roi"] = max(120, int(st["roi"]*0.92))
    ...
    if _IRIS_DBG["det_hist"]:
        total=len(_IRIS_DBG["det_hist"]); good=sum(v for _,v in _IRIS_DBG["det_hist"]); det_rate=float(good/max(1,total))
    else:
        det_rate=0.0
    _IRIS_DBG.update({"t": now, "frame_wh":(w,h), "roi_xyxy":(int(x0),int(y0),int(x1),int(y1)),
                      "cxcy":(int(cx_i),int(cy_i)), "conf":float(conf), "method":method,
                      "thr": int(thr_val), "det_rate": det_rate})
    nx=float(cx_i)/float(w); ny=float(cy_i)/float(h)
...
def _make_model():
    if Pipeline and PolynomialFeatures and Ridge:
        return (Pipeline([('poly', PolynomialFeatures(degree=2, include_bias=True)), ('reg', Ridge(alpha=1.0))]),
...
def _fit_model_dispatch(model_tuple, X, Y):
    if isinstance(model_tuple[0], str):
        ...
        pipe_x=pipe_x.fit(X, Y[:,0]); pipe_y=pipe_y.fit(X, Y[:,1])
...
def _predict_dispatch(fitted_tuple, X):
    if isinstance(fitted_tuple[0], np.ndarray) or isinstance(fitted_tuple[0], list):
        wx,wy=fitted_tuple; Phi=_poly_expand(X); x=Phi@wx; y=Phi@wy
...
class GazeEngine:
    ...
    def _apply_comp(self, x: float, y: float) -> Tuple[float,float]:
        if not (self.sess.comp_enabled and self.sess.comp_valid): return x,y
    ...
    def _stabilize(self, x:float, y:float, quality:float, eye_open:bool)->Tuple[float,float]:
        if not hasattr(self,"_stab_last"):
            self._stab_last=(0.5,0.5); self._stab_hist=[]; self._stab_last_t=time.time()
        last_x,last_y=self._stab_last; now=time.time()
        ...
        if math.hypot(x-medx,y-medy) > thr: x,y = medx,medy
        max_step=(0.08+0.06*(1.0-quality))*dt*60.0
        dx,dy=x-last_x, y-last_y; dist=math.hypot(dx,dy)
        if dist>max_step:
            sc=max_step/max(1e-6,dist); x=last_x+dx*sc; y=last_y+dy*sc
    ...
    def calibration_add_point(self, sx, sy):
        ...
        self.sess.calib_yx.append(float(sx)); self.sess.calib_yy.append(float(sy))
        q=float(self.sess.last_quality or 0.5)
        w_q=max(0.0,min(1.0,(q-0.20)/0.80))**2
        w_y=1.0 + 0.6 * abs(sy-0.5) * 2.0
        self.sess.calib_w.append(float((0.5 + 1.5*w_q)*w_y))
        return True
    def _compute_report(self, X, Y)->CalibrationReport:
        ...
        def _cv_rmse():
            ...
            if n>=9:
                k=max(2,min(5,n//3)); idx=np.arange(n); np.random.shuffle(idx)
                folds=np.array_split(idx,k); acc=[]
                for i in range(k):
                    te=folds[i]; tr=np.concatenate([folds[j] for j in range(k) if j!=i])
                    model=_make_model(); model=_fit_model_dispatch(model,X[tr],Y[tr])
                    pred=_predict_dispatch(model, X[te]); acc.append(_rmse_px(pred,Y[te],W,H))
                return float(np.mean(acc))
            else:
                reps=3; acc=[]
                for _ in range(reps):
                    idx=np.arange(n); np.random.shuffle(idx); cut=max(2,int(0.8*n))
    ...
    def _fit_compensation(self, preds: np.ndarray, targets: np.ndarray):
        # คำนวณ affine ต่อแกน: target ≈ a*pred + b (บนโดเมน normalized)
        def fit_axis(p, t):
            ...
            a=cov/var
            a=float(np.clip(a, 0.80, 1.30))  # กันหลุด
            b=float(np.mean(t) - a*np.mean(p))
            # clamp เพื่อไม่ให้เลื่อนเกินกรอบ
    ...
    def calibration_finish(self):
        ...
        if n<6:
            self.sess.model_ready=False; self.sess.model_pipeline=None; self.sess.calib_report=None
        ...
        Phi=_poly_expand(X); pen_vec=_build_penalty_vector(n_base=X.shape[1], eye_pen=0.1, face_pen=6.0, bias_pen=0.5)
        lam=1e-3
        wx=_fit_weighted_ridge(Phi, Y[:,0], Wsamples, pen_vec, lam)
        wy=_fit_weighted_ridge(Phi, Y[:,1], Wsamples, pen_vec, lam)
        self.sess.model_pipeline=(wx,wy); self.sess.model_ready=True
        # Compensation จาก residual (ใช้ prediction ก่อน apply_comp)
        preds=_predict_dispatch(self.sess.model_pipeline, X)
        self._fit_compensation(preds, Y)
        rep=self._compute_report(X,Y); self.sess.calib_report=rep
        rmse_norm=float(np.sqrt(np.mean((preds-Y)**2)))
        return {'ok':True,'rmse_norm':rmse_norm,'n':n,'rmse_px':rep.rmse_px,'rmse_cv_px':rep.rmse_cv_px,
                'uniformity':rep.uniformity,'passed':rep.passed(),
    ...
    def save_profile(self, path, meta=None):
        p=pathlib.Path(path)
        data={"calib_X":self.sess.calib_X,"calib_yx":self.sess.calib_yx,"calib_yy":self.sess.calib_yy,
              "screen":{"w":self.cfg.screen_w,"h":self.cfg.screen_h},
              "comp":{"ax":self.sess.comp_ax,"bx":self.sess.comp_bx,"ay":self.sess.comp_ay,"by":self.sess.comp_by,"valid":self.sess.comp_valid},
              "meta":meta or {}}
    ...
    def load_profile(self, path):
        ...
        try:
            ...
            sc=data.get("screen",{})
            if sc:
                self.cfg.screen_w=sc.get("w",self.cfg.screen_w); self.cfg.screen_h=sc.get("h",self.cfg.screen_h)
            comp=data.get("comp",{})
            if comp:
                self.sess.comp_ax=float(comp.get("ax",1.0)); self.sess.comp_bx=float(comp.get("bx",0.0))
                self.sess.comp_ay=float(comp.get("ay",1.0)); self.sess.comp_by=float(comp.get("by",0.0))
    ...
    def drift_add(self, sx, sy):
        ...
        ex=sx-pred[0]; ey=sy-pred[1]; a=self.sess.drift_alpha
    ...
    def _map_from_feat(self, feat_vec: np.ndarray, apply_comp: bool=True)->np.ndarray:
        ...
        eye_only=np.array([0.5+(ex-0.5)*self.cfg.fallback_kx, 0.5+(ey-0.5)*self.cfg.fallback_ky], dtype=np.float32)
    ...
    def _map_from_feat_batch(self, X, apply_comp: bool=True):
        ...
        if self.sess.model_ready and self.sess.model_pipeline is not None:
            ...
            ex,ey=X[:,0],X[:,1]
            pred=np.stack([0.5+(ex-0.5)*self.cfg.fallback_kx, 0.5+(ey-0.5)*self.cfg.fallback_ky],axis=1).astype(np.float32)
        if apply_comp:
            a=self.sess.comp_alpha
    ...
    def process_frame(self, frame_bgr):
        ...
        if feat is None:
            pred=np.array([0.5,0.5],dtype=np.float32); q=0.2; eye_open=True
        else:
            ...
            q=float(feat.get("quality",0.5)); eye_open=bool(feat.get("eye_open",True))
            self.sess.last_quality=q
        px,py=float(pred[0]),float(pred[1]); px,py=self._stabilize(px,py,q,eye_open)
        ...
        self.sess.t_prev=t1
        metrics={'fps':float(np.mean(self.sess.fps_hist)) if self.sess.fps_hist else 0.0,
                 'latency_ms':float((t1-t0)*1000.0), 'model':'calibrated' if self.sess.model_ready else 'fallback'}
    ...
    def get_report(self):
        ...
        if self.sess.model_ready and len(self.sess.calib_X)>=6:
            ...
            Y=np.stack([np.array(self.sess.calib_yx,dtype=np.float32),
                        np.array(self.sess.calib_yy,dtype=np.float32)],axis=1)
            preds=self._map_from_feat_batch(X, apply_comp=False); rmse=float(np.sqrt(np.mean((preds-Y)**2)))
    ...
    def get_comp_params(self):
        return {'enabled':self.sess.comp_enabled, 'alpha':self.sess.comp_alpha,
                'ax':self.sess.comp_ax,'bx':self.sess.comp_bx,'ay':self.sess.comp_ay,'by':self.sess.comp_by,
...
class AppState:
    def __init__(self):
        ...
        self.mouse_enabled=False; self.dwell_ms=700; self.dwell_radius=40
        self.esp32_url='http://10.63.100.193:81/stream'
        self.esp32_q=None; self.esp32_stop=None
        self.synthetic_hud=False; self.esp32_debug=False
        self.mirror_input=False; self.invert_x=False; self.invert_y=False
        self.gain=1.2; self.gamma=1.0; self.deadzone=0.02
        self.filter_mincutoff=1.0; self.filter_beta=0.01
        self.shared_gaze_x=0.5; self.shared_gaze_y=0.5
        self.calib_overlay_active=False; self.calib_targets=[]; self.calib_idx=0
        self.calib_dwell_ms=1000; self.calib_radius_norm=0.02
        self.mouse_gate_passed=False; self.mouse_gate_reason="ยังไม่คาลิเบรท"
        # NEW: Compensation UI
        self.comp_enabled=True
        self.comp_alpha=1.0
...
class GazeProcessor(VideoProcessorBase):
    def __init__(self):
        self.engine=None; self.mouse=None
        self.last_gaze=(0.5,0.5)
        self.calib_active=False; self.calib_targets=[]; self.calib_idx=0
        self.calib_hold_start=None; self.calib_radius_norm=0.02
        self._calib_banner_until=0.0; self._calib_banner_text=""
        self._feat_pool=[]
    ...
    def recv(self, frame: VideoFrame) -> VideoFrame:
        ...
        try:
            ...
            for f in (self.engine.sess.smooth_x, self.engine.sess.smooth_y):
                f.mincutoff=float(APP_STATE.filter_mincutoff); f.beta=float(APP_STATE.filter_beta)
        ...
        self.last_gaze=(x,y)
        APP_STATE.shared_gaze_x=float(x); APP_STATE.shared_gaze_y=float(y)

        # ==== Calibration flow (target-anchored: ไม่ต้องให้ crosshair ทับจุด) ====
        if APP_STATE.calib_overlay_active:
            if not self.calib_active:
                self.calib_active=True; self.calib_targets=list(APP_STATE.calib_targets)
                self.calib_idx=APP_STATE.calib_idx; self.calib_hold_start=None
                self.calib_radius_norm=float(APP_STATE.calib_radius_norm)
            tx,ty = self.calib_targets[self.calib_idx] if (0<=self.calib_idx<len(self.calib_targets)) else (0.5,0.5)
            # เงื่อนไขถือจุด: คุณภาพ + ความนิ่ง (ไม่ใช้ระยะ)
            q=float(self.engine.sess.last_quality or 0.0)
            lx,ly,lt=self._last_fix; dtv=max(1e-3, time.time()-lt)
            speed=math.hypot(x-lx,y-ly)/dtv; self._last_fix=(x,y,time.time())
            stable = (q>=0.25) and (speed<1.2)
            now=time.time()
            if stable and (self.engine.sess.last_feat is not None):
                if self.calib_hold_start is None:
                    self.calib_hold_start=now; self._feat_pool=[]
                self._feat_pool.append(self.engine.sess.last_feat.copy())
                elapsed_ms=(now-self.calib_hold_start)*1000.0
            else:
                self.calib_hold_start=None; self._feat_pool=[]; elapsed_ms=0.0
            frac=max(0.0, min(1.0, elapsed_ms/float(max(1,APP_STATE.calib_dwell_ms))))
            # วาด UI ช่วยเหลือ (แค่วงเวลา)
            if cv2 is not None:
                h,w=img.shape[:2]; gx_i,gy_i=int(tx*w),int(ty*h)
                ...
                cv2.putText(img,f"Q={q:.2f} speed={speed:.2f}",
                            (24,80), cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,200,255) if stable else (80,80,80),2)
            # commit
            if frac>=1.0 and 0<=self.calib_idx<len(self.calib_targets):
                ...
                if len(self._feat_pool)>=5:
                    ...
                    self.engine.sess.last_feat=feat_avg; ok_added=self.engine.calibration_add_point(tx,ty)
                elif self.engine.sess.last_feat is not None:
                    ok_added=self.engine.calibration_add_point(tx,ty)
                if ok_added:
                    self.calib_idx+=1; APP_STATE.calib_idx=self.calib_idx
                    self.calib_hold_start=None; self._feat_pool=[]
                if self.calib_idx>=len(self.calib_targets):
                    rep=self.engine.calibration_finish()
                    APP_STATE.calib_overlay_active=False; self.calib_active=False; self._feat_pool=[]
                    if rep.get('ok'):
            ...
            self.calib_active=False; self.calib_hold_start=None
        ...
        APP_STATE.mouse_gate_passed=bool(gate_ok); APP_STATE.mouse_gate_reason=reason

        # Mouse
        if self.mouse:
            self.mouse.set_enable(APP_STATE.mouse_enabled)
            self.mouse.dwell_ms=APP_STATE.dwell_ms; self.mouse.dwell_radius_px=APP_STATE.dwell_radius
        ...
        # ESP32 HUD
        if (APP_STATE.extractor_mode=='ESP32 (Orlosky)') and APP_STATE.synthetic_hud and APP_STATE.esp32_debug and cv2 is not None:
            dbg=globals().get("_IRIS_DBG",{})
            w0,h0=640,480; h_img,w_img=img.shape[:2]
            sx=w_img/float(w0); sy=h_img/float(h0)
            roi=dbg.get("roi_xyxy"); cen=dbg.get("cxcy")
            method=dbg.get("method","?"); conf=dbg.get("conf",0.0); thr=dbg.get("thr",-1)
            det_rate=dbg.get("det_rate",0.0)
            if roi:
                x0,y0,x1,y1=roi
                cv2.rectangle(img,(int(x0*sx),int(y0*sy)),(int(x1*sx),int(y1*sy)),(0,160,255),2)
            if cen:
                cxp,cyp=cen; cv2.circle(img,(int(cxp*sx),int(cyp*sy)),8,(255,255,0),2)
            cv2.putText(img, f"ESP32 iris [{method}] conf={conf:.2f} thr={thr} det={int(det_rate*100)}%",
                        (16,28), cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
...
def _sidebar_controls():
    ...
    APP_STATE.mouse_enabled=st.sidebar.toggle("Enable Mouse Control (pointer follows; click unlocks after calibration PASS)", value=False)
    ...
    if mode=="ESP32 (Orlosky)":
        ...
        start_esp=col_e1.button("Start"); stop_esp=col_e2.button("Stop")
        APP_STATE.synthetic_hud=col_e3.toggle("HUD w/o Webcam", value=False)
        APP_STATE.esp32_debug=st.sidebar.toggle("ESP32 Debug HUD (ROI & method)", value=False)
        if start_esp:
            if APP_STATE.esp32_stop is None:
                APP_STATE.esp32_q=queue.Queue(maxsize=4); APP_STATE.esp32_stop=threading.Event()
                t=ESP32Reader(APP_STATE.esp32_url, APP_STATE.esp32_q, APP_STATE.esp32_stop); t.start(); st.toast("ESP32 reader started")
        if stop_esp and APP_STATE.esp32_stop is not None:
            APP_STATE.esp32_stop.set(); APP_STATE.esp32_stop=None; st.toast("ESP32 reader stopped")
    ...
    if st.sidebar.button("Start Calibration"):
        ...
        APP_STATE.calib_targets=grid; APP_STATE.calib_idx=0; APP_STATE.calib_overlay_active=True
        sw,sh=_safe_size(); APP_STATE.calib_radius_norm=max(0.012, 40/max(sw,sh))
...
def main():
    ...
    st.write({"calib_active":APP_STATE.calib_overlay_active, "calib_idx":APP_STATE.calib_idx,
              "n_targets":len(APP_STATE.calib_targets), "mirror_input":APP_STATE.mirror_input,
              "invert_x":APP_STATE.invert_x, "invert_y":APP_STATE.invert_y,
              ...
              "comp_enabled": APP_STATE.comp_enabled, "comp_alpha": APP_STATE.comp_alpha})

    if ctx and ctx.video_processor and ctx.video_processor.engine:
        eng=ctx.video_processor.engine; rep=eng.get_calibration_report()
        st.markdown("---")
        ...
        st.subheader("Compensation Parameters")
        comp=eng.get_comp_params()
        st.write({"enabled": comp["enabled"], "alpha": comp["alpha"],
                  "ax": round(comp["ax"],4), "bx": round(comp["bx"],4),
                  "ay": round(comp["ay"],4), "by": round(comp["by"],4),
                  "valid": comp["valid"]})
    ...
    st.markdown("""
**ขั้นตอนใช้งาน**
1) Sidebar → เลือก 9/12 จุด → **Start Calibration** (Target-anchored: แค่มองนิ่ง ระบบจะเฉลี่ยฟีเจอร์เอง)
2) ครบทั้งหมด → ระบบฟิตโมเดล + คำนวณ Compensation (ax,bx,ay,by) อัตโนมัติ
3) เปิด "Enable Mouse Control" + "Enable Compensation" → ใช้งานจริง · คลิกจะปลดล็อกเมื่อ PASS
    """)
