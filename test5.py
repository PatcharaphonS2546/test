# test3.py
# 👁️ Vision Mouse — Pointer-only eye control (Webcam/MediaPipe only)
...
class Processor(VideoProcessorBase):
    ...
    def recv(self, frame: VideoFrame) -> VideoFrame:
        ...
        if APP.mirror and cv2 is not None:
...
# ----------------------------- Sidebar -----------------------------
def sidebar():
    st.sidebar.caption('Mode: Webcam/MediaPipe (fixed)')
...
def main():
    ...
    st.title("👁️ Vision Mouse — Eye-controlled pointer (Webcam only)")
