...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('vision_mouse_errors.log', mode='a')  # File output
    ]
)
    ...
    try:
        ...
        # ใช้ pygame สำหรับ .mp3
            ...
            try:
                ...
                log_error("pygame", e, f"playing {path}")
        ...
        log_error("play_sound_file", e, f"path={path}")
    return False
...
class MouseController:
    ...
    def update(self, x_norm: float, y_norm: float):
        ...
        try:
            ...
            log_error("MouseController", e, f"moving to ({x_px}, {y_px})")
...
class GazeEngine:
    def __init__(self, screen_w: int, screen_h: int, extractor: FeatureExtractor):
        ...
        self.gamma = 1.0
        self.deadzone = 0.02
        # Auto vertical scaling (ปรับหลังคาลิเบรตถ้าแกน Y มี span น้อย)
        self.y_gain_auto = 1.0

    def _map(self, fv: np.ndarray) -> np.ndarray:
        ex, ey = float(fv[0]), float(fv[1])
        # NOTE: ขยายแกน Y พื้นฐานมากกว่า X เล็กน้อย แต่ยังให้ polynomial เป็นตัวหลัก
        base = np.array([0.5 + (ex - 0.5) * 1.6, 0.5 + (ey - 0.5) * 3.0], dtype=np.float32)
        if self.sess.model_ready and (self.sess.wx is not None):
            ...
            px, py = base[0], base[1]
        # Apply auto Y gain (หลังคาลิเบรตอาจ >1 เพื่อชดเชย span เล็ก)
        if self.y_gain_auto != 1.0:
            py = 0.5 + (py - 0.5) * self.y_gain_auto
    ...
    def calibration_finish(self):
        ...
        # ---- Auto Y gain tuning (ไม่เพิ่ม feature UI) ----
        try:
            # ใช้ span (P95-P5) ของค่าคาดการณ์แกน Y เทียบกับ span target
            if len(preds) >= 6:
                y_pred = preds[:,1]
                y_span = float(np.percentile(y_pred, 95) - np.percentile(y_pred, 5))
                # span ที่คาดหวังจาก grid 9 จุด (min 0.2 max 0.8 ~ 0.6)
                target_span = 0.60
                if y_span > 0:
                    scale = target_span / y_span
                    # จำกัดช่วงเพื่อไม่ให้ไวเกินไป
                    self.y_gain_auto = float(min(3.5, max(1.0, scale)))
                else:
                    self.y_gain_auto = 1.0
            else:
                self.y_gain_auto = 1.0
        except Exception as e:
            log_error("calibration_finish", e, "auto y gain")
            self.y_gain_auto = 1.0
...
class AppState:
    def __init__(self):
        ...
        # Calibration: 9 points (center + 4 sides + 4 corners)
        self.targets: List[Tuple[float,float]] = [
            (0.50, 0.50),  # center
            (0.30, 0.50), (0.70, 0.50),  # left, right
            (0.50, 0.30), (0.50, 0.70),  # top, bottom  
            (0.20, 0.20), (0.80, 0.20), (0.20, 0.80), (0.80, 0.80),  # corners
        ]
        self.idx = 0
        self.dwell_ms = 1000
...
class CountdownOverlay:
    @staticmethod
    def render(img, remaining_time):
        """Render countdown overlay on frame"""
        if cv2 is None:
            return
        h, w = img.shape[:2]
        overlay = img.copy()
        ...
        sec = int(math.ceil(remaining_time))
        # ถ้าเหลือน้อยกว่า 0.3 วินาที ให้ขึ้น GO
        if 0 < remaining_time < 0.3:
            text = "GO"
        else:
            text = str(sec)
...
class CalibrationOverlay:
    @staticmethod
    def render(img, target_pos, progress_fraction, current_idx, total_targets):
        """Render calibration target with progress ring"""
        if cv2 is None:
            return
        tx, ty = target_pos
        h, w = img.shape[:2]
        ...
        # Draw progress ring
        end_angle = int(360 * progress_fraction)
        cv2.ellipse(img, (gx_i, gy_i), (22, 22), 0, 0, end_angle, (0,170,255), 3)
        
        # Draw progress text
        cv2.putText(img, f"Calibration {current_idx+1}/{total_targets}",
...
class SoundboardOverlay:
    ...
    def render(self, img, gaze_x, gaze_y, app_state):
        """Render soundboard grid with hover effects"""
        if cv2 is None:
            return None
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                inside = _inside_rect(gaze_x, gaze_y, (x0, y0, x1, y1))
                ...
                bg_colors = [
                    ...
                    (255, 255, 220), (220, 255, 255), (255, 220, 255),
                ]
                ...
                # Draw icon or text
                self._draw_cell_content(img, idx, x0_px, y0_px, x1_px, y1_px, app_state)
                
        return hover_idx
    
    def _draw_cell_content(self, img, idx, x0, y0, x1, y1, app_state):
        """Draw icon or text for a soundboard cell"""
        try:
            icon_path = app_state.sound_icons[idx] if idx < len(app_state.sound_icons) else None
            icon_w, icon_h = int((x1-x0)*0.7), int((y1-y0)*0.7)
            cx = x0 + (x1-x0)//2 - icon_w//2
            cy = y0 + (y1-y0)//2 - icon_h//2
            
            # Bounds checking
            cx = max(0, min(cx, img.shape[1]-icon_w))
            cy = max(0, min(cy, img.shape[0]-icon_h))
            
            # Try to load and cache icon
            ...
            if icon is None and icon_path and cv2 is not None:
                try:
                    ...
                    log_error("SoundboardOverlay", e, f"loading icon {icon_path}")
                    self._icon_cache[cache_key] = "MISSING"  # Cache miss to avoid repeated attempts
            
            # Draw icon if available
            if icon is not None and icon != "MISSING":
                if cy+icon_h <= img.shape[0] and cx+icon_w <= img.shape[1]:
                    if icon.shape[2] == 4:  # PNG with alpha
                        alpha = icon[:,:,3] / 255.0
                        for c in range(3):
                ...
                # Fallback to text
                label = app_state.sound_labels[idx] if idx < len(app_state.sound_labels) else f"Button {idx+1}"
            ...
            log_error("SoundboardOverlay", e, f"drawing cell {idx}")
    
    def draw_progress_ring(self, img, hover_idx, progress_fraction):
        """Draw progress ring for hovered soundboard cell"""
        if cv2 is None or hover_idx is None:
            return
        ...
        pcx = int((x0 + x1)/2 * W)
        pcy = int(y0 * H + 40)
        cv2.ellipse(img, (pcx, pcy), (28,28), 0, 0, int(360*progress_fraction), (0,200,255), 4)


def draw_crosshair(img, gaze_x, gaze_y, fps, latency_ms):
    """Draw gaze crosshair and metrics"""
    if cv2 is None:
        return
    h, w = img.shape[:2]
    ...
    cv2.putText(img, f"FPS {fps:4.1f} | Latency {latency_ms:4.1f} ms",
...
class Processor(VideoProcessorBase):
    def __init__(self):
        ...
        self.pool = []
        self.soundboard_overlay = SoundboardOverlay()
    ...
    def _acquire_frame(self, frame):
        """Get frame from webcam or ESP32 source"""
        try:
            # ESP32 mode: get frame from queue
            if APP.mode.startswith("ESP32") and hasattr(APP, 'esp_q') and APP.esp_q is not None and cv2 is not None:
            ...
            log_error("Processor._acquire_frame", e, f"mode={APP.mode}")
            return np.zeros((480, 640, 3), dtype=np.uint8)

    def _process_gaze(self, img):
        """Process gaze from current frame"""
        try:
            self._ensure()
            ...
            if APP.mode == "Webcam/MediaPipe":
                ...
                if hasattr(APP, 'esp_q') and APP.esp_q is not None:
                    try:
            ...
            m = self.engine.get_last_metrics()
            APP.ui_fps = float(m.get("fps", 0.0))
            APP.ui_lat = float(m.get("latency_ms", 0.0))
            
        except Exception as e:
            log_error("Processor._process_gaze", e, f"mode={APP.mode}")

    def _handle_countdown(self, img):
        """Handle countdown overlay before calibration"""
        if not APP.countdown_active:
            return False
        ...
        if remaining <= 0:
            ...
            # วาด countdown overlay
            CountdownOverlay.render(img, remaining)
            return True  # Skip other overlays

    def _handle_calibration(self, img):
        """Handle calibration overlay and data collection"""
        if not APP.calib_overlay:
            return
        ...
        try:
            ...
            CalibrationOverlay.render(img, (tx, ty), frac, APP.idx, len(APP.targets))
            ...
            log_error("Processor._handle_calibration", e, f"idx={APP.idx}")

    def _handle_soundboard(self, img):
        """Handle soundboard overlay and interaction"""
        if not APP.soundboard_on or APP.calib_overlay or APP.countdown_active:
            return
            
        try:
            hover_idx = self.soundboard_overlay.render(img, APP.gx, APP.gy, APP)
            ...
            if hover_idx is None:
                ...
                self.soundboard_overlay.draw_progress_ring(img, hover_idx, frac)
                ...
                if (elapsed_ms >= APP.sound_dwell_ms) and ((now - last) >= APP.sound_cooldown_ms/1000.0):
                    ...
                    if path:
                        try:
                            ...
                            log_error("Processor._handle_soundboard", e, f"playing sound {path}")
            ...
            log_error("Processor._handle_soundboard", e, "soundboard interaction")
    ...
    def _record_metrics(self):
        ...
        try:
            ...
            log_error("Processor._record_metrics", e, "recording frame metrics")

    def recv(self, frame: VideoFrame) -> VideoFrame:
        try:
            # Acquire frame
            img = self._acquire_frame(frame)
            
            # Process gaze
            self._process_gaze(img)
            
            # Handle overlays in order
            if self._handle_countdown(img):
                return VideoFrame.from_ndarray(img, format="bgr24")
                
            self._handle_calibration(img)
            self._handle_soundboard(img)
            
            # Mouse pointer control
            self.mouse.set_enable(APP.mouse_enabled)
            self.mouse.update(APP.gx, APP.gy)
            
            # Draw crosshair and metrics
            draw_crosshair(img, APP.gx, APP.gy, APP.ui_fps, APP.ui_lat)
            ...
            log_error("Processor.recv", e, "main video processing loop")
            # Return a black frame with error message
            if cv2 is not None:
                h, w = 480, 640
                error_img = np.zeros((h, w, 3), dtype=np.uint8)
                cv2.putText(error_img, f"Error: {str(e)[:50]}", (20, h//2), 
...
def sidebar():
    ...
    st.sidebar.caption("จำนวนจุดคาลิเบรต: 9 จุด (ตรงกลาง + 4 ด้าน + 4 มุม)")
    # countdown setting
    ...
    if st.sidebar.button("Start Calibration"):
        # 9-point calibration grid (center + sides + corners)
        grid = [
            (0.50, 0.50),  # center
            (0.30, 0.50), (0.70, 0.50),  # left, right
            (0.50, 0.30), (0.50, 0.70),  # top, bottom
...
- ข้อผิดพลาดถูกบันทึกไฟล์ `vision_mouse_errors.log` และ console
- สามารถตรวจสอบ log เพื่อ debug ปัญหาการโหลดไอคอน, เสียง, หรือการประมวลผลเฟรม
- หากมีปัญหา dependency (cv2, mediapipe, pygame) จะมี fallback และ log ข้อผิดพลาด
    """)

    # Show recent errors for debugging
    with st.expander("🔧 Debug Information"):
        try:
            with open('vision_mouse_errors.log', 'r') as f:
                lines = f.readlines()[-10:]  # Show last 10 errors
                if lines:
                    st.text("Recent errors (last 10):")
                    for line in lines:
                        st.text(line.strip())
                else:
                    st.text("No recent errors logged.")
        except FileNotFoundError:
            st.text("No error log file found yet.")
        except Exception as e:
            st.text(f"Could not read error log: {e}")
