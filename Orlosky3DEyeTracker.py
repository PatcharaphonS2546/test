...
STREAM_URL = "http://172.20.10.2:81/stream"  # ตัวอย่าง
...
def process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame,
    ...
    if final_rotated_rect is not None and center_x is not None and center_y is not None:
        cv2.line(frame, model_center_average, (center_x, center_y), (255, 150, 50),
                 2)  # # Draw line from eye center to ellipse center
    ...
    if GL_SPHERE_AVAILABLE:
        gl_image = gl_sphere.update_sphere_rotation(center_x, center_y, model_center_average[0],
                                                    model_center_average[1])
...
# ===== Utility: Estimate normalized pupil center from frame =====
def estimate_norm_from_frame(frame, debug_mode_on=False):
    """
    รับ frame (BGR), คืน (nx, ny) normalized pupil center ในช่วง [-0.5, +0.5]
    """
    img = crop_to_aspect_ratio(frame, width=640, height=480)
    darkest_point = get_darkest_area(img)
    if darkest_point is None:
        return None
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    thresholded = apply_binary_threshold(gray, gray[darkest_point[1], darkest_point[0]], 30)
    contours, _ = cv2.findContours(thresholded, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    reduced_contours = filter_contours_by_area_and_return_largest(contours, 1000, 3)
    if not reduced_contours or len(reduced_contours[0]) < 5:
        return None
    ellipse = cv2.fitEllipse(reduced_contours[0])
    ...
    ny = (cy / 480.0) - 0.5
    if debug_mode_on:
        print(f"Ellipse center: ({cx:.1f},{cy:.1f}) -> norm ({nx:.3f},{ny:.3f})")
    return (nx, ny)


# ===== Generator: Iterate normalized pupil center from ESP32-CAM stream =====
def iter_norm_from_esp32(url=STREAM_URL, flip_h=False, flip_v=False):
    import cv2
    ...
    while True:
        ...
        norm = estimate_norm_from_frame(frame)
        yield norm
...
def process_camera():
    ...
    while True:
        ...
        # ทิ้งเฟรมค้างคิวบ้างเพื่อลดดีเลย์ช่วงต่อใหม่
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                flip_h = not flip_h
                print(f"[TOGGLE] flip_h={flip_h}")
            elif key == ord('v'):
                flip_v = not flip_v
                print(f"[TOGGLE] flip_v={flip_v}")
            elif key == ord('s') and not paused:
                ts = int(time.time())
                cv2.imwrite(f"frame_{ts}.png", frame)
                print(f"[SAVE] frame_{ts}.png")

            # ถ้าค้างนานเกิน 3 วินาที ให้รีคอนเนค
            if time.time() - last_ok > 3.0:
                print("[TIMEOUT] No fresh frame. Reconnect …")
                break

        cap.release()
        cv2.destroyWindow("Frame with Ellipse and Rays")
        cv2.destroyWindow("Eye Tracker + Sphere")
        # วนกลับไปเปิดใหม่
...
def selection_gui():
    ...
    tk.Label(root, text=f"ESP32‑CAM URL: {STREAM_URL}").pack(pady=5)
    tk.Button(root, text="Start ESP32‑CAM", command=lambda: [root.destroy(), process_camera()]).pack(pady=8)
    tk.Button(root, text="Browse Video",   command=lambda: [root.destroy(), process_video()]).pack(pady=4)
